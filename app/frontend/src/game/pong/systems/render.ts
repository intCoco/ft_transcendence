...
import { ARENA_BOTTOM, ARENA_LEFT, ARENA_MARGIN_LEFT, ARENA_MARGIN_TOP, ARENA_RIGHT, ARENA_TOP, CANVAS_HEIGHT, CANVAS_WIDTH, GAME_HEIGHT, GameState } from "../core/constants";
...
function drawGoalsArena() {
    ...
    ctx.beginPath();
    ctx.arc(leftX - radius, GOAL_TOP + ARENA_MARGIN_TOP - radius, radius, 0, Math.PI / 2);
    ctx.stroke();
    ...
    ctx.beginPath();
    ctx.arc(leftX - radius, GOAL_BOTTOM + ARENA_MARGIN_TOP + radius, radius, -Math.PI / 2, 0);
    ctx.stroke();
    ...
    ctx.beginPath();
    ctx.arc(rightX + radius, GOAL_TOP + ARENA_MARGIN_TOP - radius, radius, Math.PI / 2, Math.PI);
    ctx.stroke();
    ...
    ctx.beginPath();
    ctx.arc(rightX + radius, GOAL_BOTTOM + ARENA_MARGIN_TOP + radius, radius, Math.PI, -Math.PI / 2);
    ctx.stroke();
...
function drawAIInfos() {
    ...
    game.ctx!.fillText(`spin: ${ball.velX}`, x, y); y += 16;
